...
remote_growth = (
  clean_pdf.groupby(["NAICS_2022_2_NAME", "POSTED_MONTH", "REMOTE_TYPE_NAME"])
  .size()
  .reset_index(name="count")
)

remote_growth["GROWTH"] = (
  remote_growth
  .groupby(["NAICS_2022_2_NAME", "REMOTE_TYPE_NAME"])["count"]
  .pct_change() * 100
)

top_10_industries = (
  clean_pdf["NAICS_2022_2_NAME"]
  .value_counts()
  .head(10)
  .index
)

top_remote_growth = remote_growth[remote_growth["NAICS_2022_2_NAME"].isin(top_10_industries)]
...
sns.lineplot(
    ...
    x="POSTED_MONTH",
    y="count",
    hue="REMOTE_TYPE_NAME",
    style="NAICS_2022_2_NAME",
    markers=True
)
plt.title("Remote Job Trends Across Top 10 Industries")
plt.xlabel("Month")
plt.ylabel("Number of Job Postings")
plt.legend(title="Remote Type / Industry", bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.show()
