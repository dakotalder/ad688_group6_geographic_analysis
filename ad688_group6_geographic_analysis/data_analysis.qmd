...
clean_pdf["TECH_HUB"] = clean_pdf["MSA_NAME"].apply(lambda x: "Tech Hub" if x in tech_hubs else "Other")

# Count of jobs by TECH_HUB
tech_hub_growth = (
    clean_pdf.groupby(["TECH_HUB"])
    .size()
    .reset_index(name="count")
)
...
sns.barplot(
    data=tech_hub_growth,
    x="TECH_HUB",
    y="count",
    palette="viridis"
)
plt.title("Job Postings: Tech Hubs vs. Other Locations")
plt.xlabel("Location Type")
plt.ylabel("Number of Job Postings")

# Add labels on top of bars
for i, row in tech_hub_growth.iterrows():
    plt.text(i, row["count"], f"{row['count']:,}", ha="center", va="bottom", fontsize=10)
